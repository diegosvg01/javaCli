...
CMD ["java","-jar","executavel_quatro-jar"]
EOF

sudo docker build . -t executavel_quatro.jar
sudo docker run -it executavel_quatro-jar

sudo docker exec -it executavel_quatro-jar bash
